...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    dist_left =  2 * np.pi * left_encoder_ticks / \
    robot_spec['encoder_resolution'] * robot_spec['wheel_radius']
    dist_right =  2 * np.pi * right_encoder_ticks / \
        robot_spec['encoder_resolution'] * robot_spec['wheel_radius']
        
    # distance travelled by the robot
    dist = (dist_left + dist_right) / 2
    
    # change in orientation of the robot
    w = (dist_right - dist_left) / (robot_spec['wheel_baseline']*2)
    
    # lateral distance travelled by the robot
    v = dist * np.sin(phi_max)
...
def histogram_update(belief, segments, road_spec, grid_spec):
    ...
    if measurement_likelihood is not None:
        ...
        # if any value is NaN, set it to 0, reset to uniform prior
        if np.any(np.isnan(belief)):
            print('NaN in belief, reset to uniform prior')
            belief[:] = 1 / belief.size

        # normalize the belief
        belief /= np.sum(belief)
        print(f'belief sum {np.sum(belief)}')
        # if the belief sum is small than a threshold, reset to  uniform prior
        sum_belief_threshold = 0.0
        if np.sum(belief) < sum_belief_threshold:
            belief[:] = 1 / belief.size
